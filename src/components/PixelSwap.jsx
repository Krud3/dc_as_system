...
// Cap the grid aggressively — each pixel clones the incoming layer.
...
const makeEasing = (value) => {
  ...
  if (!points || points.length !== 4 || points.some(Number.isNaN)) {
    const fallback = makeEasing('ease')
    easingCache.set(value, fallback)
    return fallback
  }
...
const buildGrid = ({ width, height, pixelSize, gap, pattern, randomness, maxPixels }) => {
  let size = pixelSize
  let columns = Math.max(1, Math.ceil((width + gap) / (size + gap)))
  let rows = Math.max(1, Math.ceil((height + gap) / (size + gap)))
  const pixelCap = Math.max(9, maxPixels)

  if (columns * rows > pixelCap) {
    size = Math.ceil(size * Math.sqrt((columns * rows) / pixelCap))
...
const buildKeyframes = ({ ease, startScale, endScale, spin, fade }) => {
  ...
  for (let step = 0; step <= KEYFRAME_STEPS; step += 1) {
    ...
    const angle = hasSpin ? spin * (1 - eased) : 0

    windowKf.push({
      offset: progress,
      opacity: fade ? Math.min(1, eased * 1.6) : 1,
      transform: hasSpin ? `rotate(${angle}deg) scale(${scale})` : `scale(${scale})`,
    })
    contentKf.push({
      offset: progress,
      transform: hasSpin ? `scale(${1 / scale}) rotate(${-angle}deg)` : `scale(${1 / scale})`,
    })
  }
...
function PixelSwap({
  ...
  useEffect(() => {
    ...
    const total = Math.max(160, settings.duration)
    const pixelMs = clamp(settings.pixelDuration, 50, total)
    ...
    // Clone once from the live layer, then fan out cheap detached clones.
    const template = source.cloneNode(true)
    template.dataset.visible = 'true'
    template.removeAttribute('aria-hidden')

    const animations = animationsRef.current
    const timingBase = { duration: pixelMs, easing: 'linear', fill: 'both' }
    ...
    frozenGrid.pixels.forEach((pixel, index) => {
      ...
      content.style.cssText = `left:${-pixel.left}px;top:${-pixel.top}px;width:${frozenGrid.width}px;height:${frozenGrid.height}px;transform-origin:${pixel.left + frozenGrid.size / 2}px ${pixel.top + frozenGrid.size / 2}px`

      content.appendChild(template.cloneNode(true))
      pixelElement.replaceChildren(content)

      const timing = { ...timingBase, delay: pixel.offset * spread }
  ...
  return (
    <div
      ref={containerRef}
      className={`pixel-swap${fluid ? ' pixel-swap--fluid' : ''} ${className}`.trim()}
      style={{ aspectRatio: fluid ? undefined : aspectRatio, ...style }}
