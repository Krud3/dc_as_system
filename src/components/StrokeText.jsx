...
function buildItalicIndices(text, italicWords = []) {
  ...
  for (const word of italicWords) {
    ...
    let start = 0
    while (true) {
      const idx = full.indexOf(word, start)
      if (idx === -1) break
      for (let i = 0; i < word.length; i += 1) indices.add(idx + i)
...
const StrokeText = ({
  ...
  const italicIndices = useMemo(() => buildItalicIndices(text, italicWords), [text, italicWords])
  ...
  const renderChars = (attr) =>
    characters.map((char, index) => {
      const isItalic = italicIndices.has(index)
      return (
        <tspan
          {...{ [attr]: true }}
          key={`${attr}-${index}`}
          className={isItalic ? 'stroke-text__char--italic' : undefined}
          fontStyle={isItalic ? 'italic' : undefined}
          fontFamily={isItalic ? "'Instrument Serif', Georgia, 'Times New Roman', serif" : undefined}
  ...
  }, [characters, italicIndices, fontSize, fontWeight, letterSpacing, strokeWidth])
  ...
  useEffect(() => {
    ...
    if (trigger === 'hover') {
      ...
      if (trigger === 'scroll') {
        ...
        // #root es el contenedor con overflow; window no recibe el scroll.
        if (typeof IntersectionObserver !== 'undefined') {
          ...
          io.observe(root)
          scrollTrigger = { kill: () => io.disconnect() }
        } else {
